...
import { Subscription, async, of, pipe } from 'rxjs';
import { ActivatedRoute, Router } from '@angular/router';
import { PostsService, Myfeed, MyComment } from './posts.service';
...
describe('PostsComponent', () => {
  ...
  beforeEach(async() => {
    mockValidator = jasmine.createSpyObj('PostsService', ['fData','commentData','uData']);
    await TestBed.configureTestingModule({
      declarations: [PostsComponent, FilterPipe, MatCard, MatCardContent, MatCardActions,MatCardFooter,
      MatListItem, MatList],
  ...
  beforeEach(() =>{
    ...
    mockValidator.commentData.and.returnValue(of(expectComment));
    mockValidator.fData.and.returnValue(of(expectedData));
    component = fixture.componentInstance;
    fixture.detectChanges();
    quickFilterDE = fixture.debugElement.query(By.css('#searchText'))
  })
  ...
  it('FeedForm: get Newfeed value', (done) => {
    const loginFormUserElement: HTMLInputElement = fixture.debugElement.nativeElement.querySelector('#feedForm').querySelectorAll('input')[0];
    loginFormUserElement.value = 'new feed test';
    loginFormUserElement.dispatchEvent(new Event('input'));
    fixture.detectChanges();
    fixture.whenStable().then(()=>{
      const feedValueFormGrp = component.feedForm.get('newfeed');
      expect(loginFormUserElement.value).toEqual(feedValueFormGrp.value);
      expect(feedValueFormGrp.errors).toBeNull();
      expect(component.newfeed.value).not.toBeNull();
      done();
    })
  });

  it('FeedForm: Check documents after Add post() ', ()=>{
    const loginFormUserElement: HTMLInputElement = fixture.debugElement.nativeElement.querySelector('#feedForm').querySelectorAll('input')[0];
    let crtlen = component.documents.length;
    loginFormUserElement.value = 'new feed test';
    loginFormUserElement.dispatchEvent(new Event('input'));
    fixture.detectChanges();
    component.addfeed();
    expect(component.documents.length-crtlen).toEqual(1);
  })

  it('should fetch all articles for current logged in user (post refers to documents)', async() => {
    expect(component.documents.length).toEqual(10)
  });

  it('Check add and delete feed by function (posts length becomes larger and smaller))', async() => {
    mockValidator.fData.and.returnValue(of(expectedfollow));
    component.searchfeedfol(2,'Ervin Howell');
    expect(component.documents.length).toEqual(20)

    component.deletefollowfeed(expectprofile);
    expect(component.documents.length).toEqual(10)
  });

  it('Check ngFor* filter: check for current mat-card in DOM', fakeAsync(() => {
